    ...
    class DataBaseConnection {
        ...
        public function updateMovie($movieid, $title, $duration, $releaseyear, $rating, $userid) {
            ...
            if (!$result) {
                echo "Error fetching username: " . mysqli_error($this->conn);
                return false;
            }

            $row = mysqli_fetch_assoc($result);
            $query2 = "UPDATE movies SET title='$title', duration='$duration', releaseyear='$releaseyear', rating='$rating', addedby='$userid' WHERE movieid='$movieid'";
            $complete = mysqli_query($this->conn, $query2);
        ...
        public function get_Users() {
            $query = "SELECT * FROM users WHERE accountType != 1" ;
            $results = mysqli_query($this->conn, $query);
